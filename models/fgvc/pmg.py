...
class PMG(nn.Module):
    def __init__(self, args, model, feature_size, classes_num):
        ...
        self.classifier_concat_block = nn.Sequential(
        ...
        self.classifier_conv_block1 = nn.Sequential(
        ...
        self.classifier_conv_block2 = nn.Sequential(
            nn.BatchNorm1d(self.num_ftrs//2),
            nn.Linear(self.num_ftrs//2, feature_size),
            nn.BatchNorm1d(feature_size),
            nn.ELU(inplace=True),
        ...
        self.classifier_conv_block3 = nn.Sequential(
            nn.BatchNorm1d(self.num_ftrs//2),
            nn.Linear(self.num_ftrs//2, feature_size),
            nn.BatchNorm1d(feature_size),
            nn.ELU(inplace=True),
    ...
    def forward(self, x):
        ...
        xc_1 = self.classifier_conv_block1(xl1)
        xc1 = self.classifier1(xc_1)

        xl2 = self.max2(xl2)
        xl2 = xl2.view(xl2.size(0), -1)
        xc_2 = self.classifier_conv_block2(xl1)
        xc2 = self.classifier2(xc_2)

        xl3 = self.max3(xl3)
        xl3 = xl3.view(xl3.size(0), -1)
        xc_3 = self.classifier_conv_block3(xl1)
        xc3 = self.classifier3(xc_3)

        x_concat = torch.cat((xl1, xl2, xl3), -1)
        temp = self.classifier_concat_block(x_concat)
        x_concat = self.classifier_concat(temp)

        return xc1, xc2, xc3, x_concat, xc_1, xc_2, xc_3, temp
